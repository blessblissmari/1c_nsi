...
export function GlassCard({ children, className = '', hover = false, onClick }: GlassCardProps) {
  return (
    <motion.div
      ...
      whileHover={hover ? { scale: 1.02, boxShadow: '0 0 30px rgba(37,99,235,0.15)' } : {}}
...
export function GlassPanel({ children, className = '', title, action }: GlassPanelProps) {
  return (
    <div className={`glass-panel p-4 ${className}`}>
      {(title || action) && (
        <div className="flex items-center justify-between gap-3 mb-4 min-w-0">
          {title && <h3 className="text-base font-semibold text-slate-800 truncate min-w-0">{title}</h3>}
          {action}
        </div>
      )}
      {children}
...
export function ActionButton({ label, onClick, loading, disabled, variant = 'primary', icon, size = 'md' }: ActionButtonProps) {
  const baseClass = variant === 'primary' ? 'btn-primary' : variant === 'danger' ? 'btn-primary !bg-error' : 'btn-secondary'
...
interface DocumentPreviewModalProps {
  document: { filename: string; parsed_content?: string | null; file_type?: string } | null
  onClose: () => void
}
...
export function DocumentPreviewModal({ document, onClose }: DocumentPreviewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="glass-panel w-full max-w-3xl max-h-[80vh] m-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-glass-border">
          <div className="flex items-center gap-2">
            <FileText size={18} className="text-neon" />
            <h3 className="text-lg font-semibold text-text-primary truncate">{document.filename}</h3>
            {document.file_type && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-neon/10 text-neon">{document.file_type}</span>
            )}
          </div>
        ...
        <div className="p-4 overflow-y-auto max-h-[calc(80vh-60px)]">
          {document.parsed_content ? (
            ...
            <p className="text-text-muted text-sm text-center py-8">Содержимое документа не доступно</p>
          )}
        </div>
      </div>
    </div>
...
export function DocumentPreviewButton({ filename, hasContent, onClick }: DocumentPreviewButtonProps) {
